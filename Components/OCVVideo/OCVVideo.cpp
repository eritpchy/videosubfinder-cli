...
HRESULT OCVVideo::GetPin(IBaseFilter *pFilter, PIN_DIRECTION PinDir, IPin **ppPin)
{
...
HRESULT OCVVideo::ConnectFilters(IGraphBuilder *pGraph,IBaseFilter *pFirst,IBaseFilter *pSecond)
{
    HRESULT hr;
...
bool OCVVideo::OpenMovieAllDefault(string csMovieName, void *pHWnd)
{ 	
	/*HRESULT hr;
	string Str;
	vector<CLSID> cls;
...
bool OCVVideo::OpenMovieNormally(string csMovieName, void *pHWnd)
{ 	
	HRESULT hr;
	string Str;
	vector<CLSID> cls;
...
bool OCVVideo::OpenMovieHard(string csMovieName, void *pHWnd)
{ 	
	m_type = -3;
    return OpenMovieNormally(csMovieName, pHWnd);
}

bool OCVVideo::SetVideoWindowPlacement(void *pHWnd)
{
	HRESULT hr;
	bool result = false;
	CLSID classID;
...
IBaseFilter* OCVVideo::GetDecoder()
{
	HRESULT hr;
	IPin *pOut, *pIn;
	PIN_INFO PinInfo;
...
IBaseFilter* OCVVideo::GetSourceFilter()
{
	HRESULT hr;
	IPin *pOut, *pIn;
	PIN_INFO PinInfo;
		...
		else
		{
				...
				(PrevPos < (m_Duration - ddt*2)) )
			{
				...
				if (m_ImageGeted == false)
				{
					CurPos = PrevPos;
					bln = 1;
					//MessageBox(NULL, "Can'nt get new image.", "OneStepWithTimeout in OCVVideo", MB_ICONERROR);
				}
			}
			...
			if (bln == 0)
			{
				...
				if ( (tmpPos == PrevPos) || ((tmpPos - PrevPos) > ddt) )
				{
					...
					{
						//MessageBox(NULL, "Can'nt get image with pos != prev_pos.", "OneStepWithTimeout in OCVVideo", MB_ICONERROR);
					}
				}
...
s64 OCVVideo::PosToMilliSeconds(s64 pos)
{
